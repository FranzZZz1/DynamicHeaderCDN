class HeaderScrollOffset {
    ...
    #setIsHeaderHidingDestroyedValue() {
        if (this.headerHidingInstance) {
            ...
            if (
                typeof this.headerHidingInstance.onDestroyCallback !==
                "undefined"
            ) {
                this.headerHidingInstance.onDestroyCallback = (value) => {
                    this._isHeaderHidingDestroyed =
                        this.#isHeaderHidingDestroyed(value);
                };
            }
        } else {
    ...
    #initVariables() {
        ...
        if (!this.anchorLinks) {
            return this.errors.push(
                this.root.moduleName + ": There are no links on the page"
            );
        }
        ...
        this.anchorLinks.forEach((link) => {
            if (link.getAttribute("href") === "#") {
                ...
                const handleHeightAccounting = (event) =>
                    this.#scrollToElement(link, event);
    ...
    #scrollToElement(link, event) {
        ...
        if (targetElement) {
            ...
            const scrollOptions = this.#calculateScrollOptions(
                offsetTop,
                link,
                targetElement
            );
    ...
    #calculateScrollOptions(offsetTop, link, targetElement) {
        ...
        const headerHidingDisabled =
            !this.headerHiding || this._isHeaderHidingDestroyed;
        const headerHidingEnabled =
            this.headerHiding && !this._isHeaderHidingDestroyed;
            ...
            if (headerHidingDisabled || (headerHidingEnabled && isFirstLink)) {
                if (isHeaderFixed) {
                    if (isFirstLink) {
                        ...
                        scrollOptions.top =
                            offsetTop - this.headerHeight - this.scrollMargin;
                    }
                } else {
                    if (isFirstLink) {
                        scrollOptions.top =
